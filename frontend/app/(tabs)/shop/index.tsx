...
const Shop = () => {
  ...
  const addToCart = (item: Item) => {
    if (!item.fromShop && cart.some((cartItem) => cartItem.id === item.id)) {
      return;
    }
  ...
  return (
    <View className="flex-1">
      <TouchableOpacity onPress={() => setCartVisible(!cartVisible)} className="fixed top-28 left-5 z-50">
            ...
            renderItem={({ item }) => {
              ...
              return (
                <View className="bg-white rounded-xl p-4 m-2 w-40 items-center shadow-lg justify-between">
                  ...
                  <TouchableOpacity className={`px-3 py-2 rounded-lg mt-2 ${disableAdd ? "bg-gray-400" : "bg-blue-500"}`} onPress={() => addToCart(item)}>
